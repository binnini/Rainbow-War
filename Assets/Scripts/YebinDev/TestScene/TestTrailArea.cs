...
public class TestTrailArea : MonoBehaviour
{
	public static int team;	
	public GameObject mainTrail;
	public static List<List<Vector3>> attackLineList;
    public MeshCollider coll;		// 모양이 사각형이라 충돌 오류 발생 여지가 있음. sphereCollider로 변경하든 모양이 바꾸든 해야함.
	...
	public static List<Vector3> DeformTrailArea(TestTrail_Action trail, List<List<Vector3>> newAreaVerticesList,TestTrail_Action attackTrail, List<Vector3> attackVertices)
	{		
		...
		if (newAreaVertices.Count > 0)
		{
			...
			// tempredundantVertices를 이용하여 시계 영역이 공격 영역에 포함되는지 체크
			if (attackVertices != null || attackTrail != null) {
				...
				// for (int i=0; i <tempRedundantVertices.Count; i++) {
				// 	 Debug.Log("시계방향의 " +i + "번째 redundant X :  " + tempRedundantVertices[i].x + " Z : " + tempRedundantVertices[i].z);
				// }
				...
				{									
					...
					// if (TrailArea.IsPointInPolygon(new Vector2(tempRedundantVertices[i].x, tempRedundantVertices[i].z), trail.Vertices2D(attackVertices))) {						
					// 	Debug.Log("cw의 " + i + "번째가 포함됨. X :  " + tempRedundantVertices[i].x + " Z : " + tempRedundantVertices[i].z);
					// }
			...
			if (attackVertices != null || attackTrail != null) {
				...
				// for (int i=0; i <tempRedundantVertices.Count; i++) {
				// 	 Debug.Log("반시계방향의 " +i + "번째 redundant X :  " + tempRedundantVertices[i].x + " Z : " + tempRedundantVertices[i].z);
				// }
				for (int i=1; i<tempRedundantVertices.Count-1; i++) 
				{									
					...
					// if (TrailArea.IsPointInPolygon(new Vector2(tempRedundantVertices[i].x, tempRedundantVertices[i].z), trail.Vertices2D(attackVertices))) {						
					// 	Debug.Log("ccw의 " + i + "번째가 포함됨. X :  " + tempRedundantVertices[i].x + " Z : " + tempRedundantVertices[i].z);
					// }
